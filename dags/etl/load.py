...
def load_interest_over_time(cursor, connection, data, keyword, channel, schedule_interval):
    try:
        value_col = [col for col in data.columns if col not in ("index", "date", "isPartial")][0]
        rows = [
            (row["date"], keyword, row[value_col], channel, schedule_interval)
            for _, row in data.iterrows()
        ]
        insert_query = """
        INSERT INTO interest_over_time (date, keyword, value, channel, schedule_interval)
        VALUES (%s, %s, %s, %s, %s)
...
def load_interest_by_region(cursor, connection, data, keyword, schedule_interval):
    try:
        ...
        rows = [
            (row["geoName"], keyword, row[value_col], schedule_interval, today_date)
            for _, row in data.iterrows()
        ]
        insert_query = """
        INSERT INTO interest_by_region (region, keyword, value, schedule_interval, load_date)
        VALUES (%s, %s, %s, %s, %s)
        ON CONFLICT DO NOTHING;
        """
        cursor.executemany(insert_query, rows)
